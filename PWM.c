...
const unsigned char sinewave_data[]= {
	...
	224, 230, 236, 242, 248, 254, 255, 254, 248, 242, 236, 230, 224, 218, 212, 206,
	200, 194, 188, 182, 176, 170, 164, 158, 152, 146, 140, 134, 128, 122, 116, 110,
	104, 98, 92, 86, 80, 74, 68, 62, 56, 50, 44, 38, 32, 26, 20, 14, 8, 2, 1, 2, 8,
	14, 20, 26, 32, 38, 44, 50, 56, 62, 68, 74, 80, 86, 92, 98, 104, 110, 116, 122
};
...
void incrementSin(){
	static char index = 0;
	setDutyCycle_1A(sinewave_data[index%80]);
	setDutyCycle_1B(sinewave_data[(index+26)%80]);
	setDutyCycle_2B(sinewave_data[(index+52)%80]);
	index++;
}
...
void PWM_init(char NmbrPWM){
	switch(NmbrPWM){
		case 1:
			PWM_1_A_init(0x5,255);
			break;
		case 2:
			PWM_1_A_B_initOPPOSEE(0x5,255);
			break;
		case 3:

...
void PWM_1_A_B_initTriangle(unsigned char Prescaler, unsigned int Top_1)
{
	...
	//Clear OC1A, OC1B on Compare Match, set OC1A, OC1B at BOTTOM (non-inverting mode) --> see Table 14-3
	//fast PWM --> see Table 14-5 , mode = 14
	TCCR1A  |= ((1 << COM1A1) | (1 << COM1B1) | (1 << COM1B1) | (1 << WGM11));
	TCCR1B |=  (1 << WGM13);
	// Set prescaler, see Table 14-6
	Tampon = TCCR1B & 0B11111000;
	TCCR1B = Tampon | Prescaler;
...
void PWM_ManualMode_Initialisation(void){
	if (DentDeScie)
	{
		...
		switch(NombrePWM){
			case 1:
			PWM_1_A_initTriangle(0x5,PWMPeriode);
			break;
			case 2:
			PWM_1_A_B_initTriangle(0x5,PWMPeriode);
			break;
			case 3:
			PWM_1_A_B_initTriangle(0x5,PWMPeriode);
			PWM_2_B_initTriangle(0x7,PWMPeriode); // TOP = 255; 8 bits
			break;
		}
	}
}
